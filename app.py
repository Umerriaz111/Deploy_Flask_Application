...
def getresponses(questions,data):
    prompt = f"""
    You are a helpful assistant. Based on the provided data, generate a response to the following user query.
    ...
    """
    response = client.text_generation(prompt, max_new_tokens=3000, temperature=0.5, top_p=0.9)
...
def ask_question_about_link( user_query=None):
    ...
    link="https://en.wikipedia.org/wiki/United_States_presidential_election"
